...
export const ActivitiesAll: FunctionalComponent = () => {
  ...
  const locationsIDs = Array.from(
    new Set(
      activitiesStore.Activities.filter((activity) => activity.day === day).map(
  ...
  let currentPlace =
    router.query.place &&
    locationsIDs.some((locationId) => locationId === router.query.place)
      ? router.query.place
      : locationsIDs[0];
  ...
  const activities = useCell(() =>
    activitiesStore.Activities.filter((activity) => activity.day === day)
  );
  ...
  return (
    <>
      ...
      <div flex column gap={2} style={{ margin: "25px 0 20px 0" }}>
        <Tags value={day} tagsList={filters}>
          {({ key, value }) => (
            <Tag
        ...
        {currentFilter === "time" ? (
          ...
          <Tags style={"flex-wrap: wrap;"} tagsList={locationsIDs}>
            {(locationId) => (
              <Tag
