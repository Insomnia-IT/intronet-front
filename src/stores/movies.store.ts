...
export class MovieBlockStore {
  constructor(public id: string, public day: number, public locationId: string) {
  }
  ...
  get duplicate() {
    const duplicate = this.block.views.find((x) => x.day !== this.day || x.locationId !== this.locationId);
    ...
    if (isAfter) {
      return `Покажем этот блок ещё раз ${getDayText(duplicate.day, "at")} в ${duplicate.start} на ${screenAt}`;
    }
    return `Этот блок шёл ${getDayText(duplicate.day, "at")} в ${duplicate.start} на ${screenAt}`;
  }

  public state = new Cell(() => ({
    block: this.block,
    duplicate: this.duplicate,
    view: this.block.views.find((x) => x.day == this.day && x.locationId == this.locationId),
  }));
}
