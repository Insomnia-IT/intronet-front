...
export function Food() {
  ...
  return <PageLayout title='Еда' withCloseButton>
...
const sections = {
  food: {
    ...
    component: () => <>
      ...
      <div>
        Рядом с кафе организованы места с проточной водой и чистящими средствами, чтобы мыть свою посуду.
      </div>
      <ButtonsBar at="bottom">
        <Button type="blue" class="w-full" goTo={['map', { direction: Directions.cafe }]}>все кафе на карте</Button>
      </ButtonsBar>
    </>
  },
  fire: {
    ...
    component: () => <>
      <div>
        Сейчас в России действуют строгие правила пожарной безопасности. На походные газовые горелки в этом законе прямого запрета нет.
      </div>
      <div>
        Если случилась беда — <b>подходите к волонтёрам с рацией</b>. На территории работают службы, которые помогут.
      </div>
      <div>
        Ещё на поле есть два костра у Фудкорта и у Детского лагеря. Тут нельзя готовить, но можно отдохнуть и погреться!
      </div>
      <Link goTo="/map" query={{ name: 'костер' }}>к костру у фудкорта</Link>
      <Link goTo="/map" query={{ name: 'костер' }}>к костру у детской поляны</Link>
