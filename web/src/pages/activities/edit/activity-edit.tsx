...
import {
  activitiesStore,
  ActivityStore,
} from "../../../stores/activities/activities.store";
...
export const ActivityEdit = ({ mode }: ActivityEditProp) => {
  ...
  const cell = useMemo(
    () =>
      new Cell(() => {
        const activity: Activity = activitiesStore.Activities.find(
          (x) => x._id === router.route[2]
        );
        return (
          activity ?? {
            _id: Fn.ulid(),
  ...
  return (
    <div flex column gap={4}>
      <PageHeader
        titleH2={mode === "create" ? "Создание" : "Редактирование"}
        align={"top"}
        withCloseButton
      />
      ...
      {mode !== "time" && (
        <form ref={ref} flex column gap={2}>
          ...
          >
            {(d) => (
              <Tag
                selected={d._id === activity.locationId}
                key={d}
                onClick={() => {
      ...
      {mode !== "create" && (
        ...
        >
          отменить {activity.isCanceled ? "отмену мероприятия" : "мероприятие"}
        </Button>
      )}
