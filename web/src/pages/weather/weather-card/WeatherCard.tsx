    ...
    <Card className={classNames(styles.weatherCard, styles.weatherCardExpanded)} background='Soft' onClick={onClick}>
      ...
      <div className={styles.timesOfDay}>
        {Object.entries(timesOfDay).map(([key, timeOfDay]) => <div key={key} className={styles.timeOfDay}>
          <div className={styles.time}>{getTimeName(key as 'morning' | 'afternoon' | 'evening' | 'night')}</div>
          <div className={styles.temperature}>{timeOfDay.temperature > 0 ? '+' : ''}{timeOfDay.temperature}°C</div>
          <div className={styles.condition}>{timeOfDay.condition}</div>
        </div>)}
      </div>
    </Card>
  )
